...
void main()
{
	...
	//float dist = distance(vTexPosition, vec2(0.5, 0.5));

	value_L = get_data_pixel(uSampler_L, uDataTextureSize, vTexPosition[0]);
	value_R = get_data_pixel(uSampler_R, uDataTextureSize, vTexPosition[1]);

	float diff = 0.5 + (value_L - value_R) * 0.25;
	float diff2 = abs(value_L - value_R);

	float abs_L = abs(0.5 - value_L) * 2.0;
	float abs_R = abs(0.5 - value_R) * 2.0;

	float value_max = max(abs_L, abs_R);
	float value_min = min(abs_L, abs_R);

	//float alpha = 1.0 * max(min((0.5 - dist) * 2.0, 1.0), 0.0);

	//diff = 1.0 - pow(1.0 - diff, 2.0);

	gl_FragColor[0] = diff;
	gl_FragColor[1] = 1.0;
	gl_FragColor[2] = 1.0;
	gl_FragColor[3] = 1.0; //  - pow(1.0 - alpha, 6.0);
