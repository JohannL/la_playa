...
µ.WebGL_Rectangle_Textured_Bar = function(webgl, gl, cameras, textures, framebuffer_texture_size)
{
...
(function () {/*
	...
	{
		tex_pos_y = (0.5 - vTexPosition[0]) * 2.0;
	}
	else
	{
		tex_pos_y = (vTexPosition[0] - 0.5) * 2.0;
	}
	tex_pos_y = pow(tex_pos_y, 1.75);

	//

	tex_pos_y = vTexPosition[0];

	vec4 texSpectrogram = texture2D(uSampler, vec2(tex_pos_y, vTexPosition[1]));

	gl_FragColor = texSpectrogram;
	if (vTexPosition[0] < 0.5)
	{
	...
	{
		gl_FragColor[0] = gl_FragColor[1];
	}

	gl_FragColor[1] = 1.0;
	gl_FragColor[2] = 1.0;					// oscilloscope stuff isn't used for this, use it for luminance
	gl_FragColor[3] = 1.0;

	if (gl_FragColor[0] < vTexPosition[1])
	{
		// fade out
		gl_FragColor[0] = 0.0;
		gl_FragColor[1] = 0.0;
	...
	float thickness = 0.2 + 1.5 * (0.0625 - pow(vTexPosition[0] - 0.5, 4.0));

	if (abs(texSpectrogram[2] - (vTexPosition[1] * 2.0 - 1.0)) < thickness )
	{
		float fade = abs(texSpectrogram[2] - (vTexPosition[1] * 2.0 - 1.0)) / thickness;
		gl_FragColor[0] = 1.0 - fade;
		gl_FragColor[1] = 1.0 - fade;
		gl_FragColor[2] = 1.0 - fade;
		gl_FragColor[3] = 1.0;
	}
	else
	{
