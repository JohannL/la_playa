...
µ.WebGL_Rectangle_Textured_Vis = function(gl, cameras, textures)
{
...
(function () {/*
	...
	void main()
	{
		...
		float uTextureStep = 1.0 / uTextureSize;
		float uTextureStep2 = uTextureStep * 0.5;

		vec2 tex_pos = vec2(vTexPosition[0] * vTexScale[0], vTexPosition[1] * vTexScale[0]);

//		vec2 pixel = floor(tex_pos * uTextureSize);
//		vec2 pixel_offset = (tex_pos * uTextureSize - pixel);

	    vec2 f = fract( tex_pos * uTextureSize );

	    float texture_step_x = f.x >= 0.5 ? uTextureStep : -uTextureStep;
	    float texture_step_y = f.y >= 0.5 ? uTextureStep : -uTextureStep;

//    	texture_step_x = (tex_pos[0] + texture_step_x >= 1.0 || tex_pos[0] + texture_step_x <= 0.0) ? 0.0 : texture_step_x;
//    	texture_step_y = (tex_pos[0] + texture_step_y >= 1.0 || tex_pos[0] + texture_step_y <= 0.0) ? 0.0 : texture_step_y;

	    f.x = f.x >= 0.5 ? (1.0 - (f.x - 0.5)) : (0.5 + f.x);
	    f.y = f.y >= 0.5 ? (1.0 - (f.y - 0.5)) : (0.5 + f.y);

	    vec4 tl = texture2D(uSampler, tex_pos);
	    vec4 tr = texture2D(uSampler, tex_pos + vec2(texture_step_x, 0.0));
	    vec4 bl = texture2D(uSampler, tex_pos + vec2(0.0, texture_step_y));
	    vec4 br = texture2D(uSampler, tex_pos + vec2(texture_step_x, texture_step_y));
	    vec4 tA = mix( tr, tl, f.x);
	    vec4 tB = mix( br, bl, f.x);
	    vec4 texColor = mix( tB, tA, f.y);

		//float frac = texColor[0]; // clamp(texColor[0], 0.0, 1.0);
		
		if (vColor[2] < 0.0)
		{
			gl_FragColor = apply_color_scheme2(vec4(texColor[0], texColor[1], vColor[2] * texColor[2], texColor[3] * vColor[3]));
		}
		else
		{
			gl_FragColor = apply_color_scheme(vec4(texColor[0], texColor[1], vColor[2] * texColor[2], texColor[3] * vColor[3]));
		}
		
		//gl_FragColor[3] = pow(gl_FragColor[0], 0.1);
...
*/
	...
	this.program.uVolume			= gl.getUniformLocation(this.program, "uVolume");
...
//*/
	...
	this.flush_all = function(texture_size, volume)
	{
		...
		//for (var camera_id in cameras)
		{
			...
			{
				this.flush(camera_id, texture_id, texture_size, volume);
			}
		}
	...
	this.flush = function(camera_id, texture_id, texture_size, volume)
	{
		...
		gl.uniform1f(this.program.uTextureSize, texture_size);
		gl.uniform1f(this.program.uVolume, 	volume);
