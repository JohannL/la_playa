...
µ.WebGL_Rectangle_Textured_Phase_Dist = function(webgl, gl, cameras, textures, framebuffer_texture_size)
{
...
void main()
{
	...
	float diff = pow(abs(value_L - value_R), 0.1);

	//float dist = distance(vTexPosition, vec2(0.5, 0.5));
	//float alpha = diff * max(min((0.5 - dist) * 2.0, 1.0), 0.0);

	gl_FragColor[0] = diff;
	gl_FragColor[1] = 1.0 - min(value_L, value_R);
	gl_FragColor[2] = 0.85 + 0.075 * min(value_L, value_R) + 0.075 * max(value_L, value_R);
	gl_FragColor[3] = 1.0 - pow(1.0 - diff, 2.0);
