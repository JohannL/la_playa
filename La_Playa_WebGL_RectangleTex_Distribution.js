...
µ.WebGL_Rectangle_Textured_Distribution = function(webgl, gl, cameras, textures, framebuffer_texture_size)
{
	...
	this.vertex_shader = [(function () {/*
// Rectangle_Textured_Distribution
...
void main()
{
	...
	float value_there  = 0.0;

	float total_data_pixels = uDataTextureSize * uDataTextureSize * 4.0;
	float texture_step = 1.0 / total_data_pixels;

	float scale_y = 1.0;
	float scale_x = 7.0; // this makes it smaller

	float tex_pos_y = ((vTexPosition[1] - 0.5) * scale_y) + 0.5;
	
	float amplify = 0.5;

	float frac = fract(tex_pos_y * total_data_pixels);

	float step_direction = 0.0;
	if (frac >= 0.5)
	{
		step_direction = texture_step;
		frac = 1.5 - frac;
	}
	else
	{
	...
	{
		pos_x = (1.0 - (1.0 - (vTexPosition[0] * 2.0))) * scale_x;
		value_here = pow(get_data_pixel(uSampler_L, uDataTextureSize, tex_pos_y), amplify);
		value_there = pow(get_data_pixel(uSampler_L, uDataTextureSize, tex_pos_y + step_direction), amplify);
		value_here = mix(value_there, value_here, frac);
	}
	else
	{
		pos_x = (1.0 - (vTexPosition[0] - 0.5) * 2.0) * scale_x;
		value_here = pow(get_data_pixel(uSampler_R, uDataTextureSize, tex_pos_y), amplify);
		value_there = pow(get_data_pixel(uSampler_R, uDataTextureSize, tex_pos_y + step_direction), amplify);
		value_here = mix(value_there, value_here, frac);
	}

	float temp_hue = value_here;
	float fade_edges = (1.0 - pow((tex_pos_y - 0.5) * 2.0, 2.0));
	float fade_edges2 = (1.0 - pow((tex_pos_y - 0.5) * 2.0, 8.0));

	float thickness = pow(value_here, 1.5) * 4.0;
	
	//thickness = 0.1;

	float dist = max(0.0, (pos_x - value_here) / (1.0 - value_here));

	if (dist < thickness )
	{
		float fade = pow(1.0 - dist / thickness, 0.015);
		gl_FragColor[0] = fade * fade * fade * fade;
		gl_FragColor[1] = 1.0;
		gl_FragColor[2] = 1.0;
		gl_FragColor[3] = fade * 1.0 * fade_edges2;
	}
	
	//gl_FragColor[0] += ceil(value_here) * 0.25;
	...
	gl_FragColor[3] = clamp(gl_FragColor[3] * 1.0, 0.0, 1.0);
...
*/
	...
	gl.uniform1f(this.program.uDataTextureSize, 	8);
...
//*/
	...
	this.flush_all = function(texture_L_id, texture_R_id)
	{
		...
		//for (var camera_id in cameras)
		{
			...
			{
				this.flush(camera_id, texture_id, texture_L_id, texture_R_id);
			}
		}
	...
	this.flush = function(camera_id, texture_id, texture_L_id, texture_R_id)
	{
