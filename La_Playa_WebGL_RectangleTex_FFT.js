...
µ.WebGL_Rectangle_Textured_FFT = function(webgl, gl, cameras, textures)
{
...
void main()
{
	...
	value_here = 1.0 - clamp(value_here / uMinDecibels, 0.0, 1.0);
	gl_FragColor[0] = value_here;
	gl_FragColor[1] = 1.0;
	gl_FragColor[2] = 1.0;
	gl_FragColor[3] = 1.0;
	float pos_y = vTexPosition[1];
	float dist = abs(pow(vTexPosition[1], 1.0) - (1.0 - gl_FragColor[0]));
	float dist2 = abs(pow(vTexPosition[1], 1.0) - (gl_FragColor[0]));
	if ((1.0 - gl_FragColor[0]) > pow(vTexPosition[1], 1.0))
	{
		gl_FragColor[0] *= pow(1.0 - dist, 2.0);
		gl_FragColor[2] *= pow(1.0 - dist, 6.0);
	}
	else
	{
		float fade_y = pow(vTexPosition[1], 8.0);
		gl_FragColor[0] -= pow(1.0 - dist, 5.0) * 0.025 + pow(1.0 - dist2 * gl_FragColor[0], 7.0) * 0.125;
		gl_FragColor[2] -= pow(1.0 - dist, 8.0) * 0.025 + pow(1.0 - dist2 * gl_FragColor[0], 16.0) * 0.125;
	}
	float pos_y_pow = pow(pos_y, 1.0 + 7.0 * value_here);
	gl_FragColor[0] = gl_FragColor[0] * (1.0 - pos_y_pow) + pos_y_pow * value_here;
	gl_FragColor[2] = gl_FragColor[2] * (1.0 - pos_y_pow) + pos_y_pow;
...
*/
	...
	gl.errorCheck('RectangleTex_FFT');
