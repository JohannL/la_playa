...
µ.WebGL_Rectangle_Textured_Phase = function(webgl, gl, cameras, textures, framebuffer_texture_size)
{
...
void main()
{
	float dist = distance(vTexPosition, vec2(0.5, 0.5)) * 2.0;
	if (dist > 1.0)
	{
		discard;
	}
	float angle_frac = mod(xy_to_angle(0.5 - vTexPosition[0], 0.5 - vTexPosition[1]) + 90.0, 360.0) / 360.0;


	float diff = 0.0;

	if (vTexPosition[0] >= 0.0)
	//if (mod(angle_frac, 0.25) <= 0.125)
	{
		float value_L = get_data_pixel(uSampler_L, uDataTextureSize, angle_frac * 0.5);
		float value_R = get_data_pixel(uSampler_R, uDataTextureSize, dist * 0.5);
		diff = 0.5 + (value_L - value_R) * 0.25;
		//diff = abs(value_L - value_R);
	}
	else
	{
		float value_R = get_data_pixel(uSampler_R, uDataTextureSize, angle_frac * 0.5);
		float value_L = get_data_pixel(uSampler_L, uDataTextureSize, dist * 0.5);
		diff = 0.5 + (value_L - value_R) * 0.25;
		//diff = abs(value_L - value_R);
	}
	...
	gl_FragColor[1] = 1.0;
	gl_FragColor[2] = 1.0;
	gl_FragColor[3] = 1.0 - pow(dist, 16.0);

	gl_FragColor = apply_color_scheme(gl_FragColor);
...
*/
	...
	this.flush_all = function(texture_L_id, texture_R_id, data_texture_size)
	{
		...
		//for (var camera_id in cameras)
		{
			...
			{
				this.flush(camera_id, texture_id, texture_L_id, texture_R_id, data_texture_size);
			}
		}
	...
	this.flush = function(camera_id, texture_id, texture_L_id, texture_R_id, data_texture_size)
	{
		...
		gl.uniform1f(this.program.uDataTextureSize, 	data_texture_size);
